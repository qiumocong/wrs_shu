...
import robot_sim.end_effectors.handgripper.dexgripper.dexgripper_4 as hnd
...
if __name__ == '__main__':
    ...
    # robot_s.hnd.dif_grabbing1()
    robot_s.gen_meshmodel(toggle_tcpcs=True, toggle_jntscs=False).attach_to(base)
    tgt_pos = np.array([.25, .2, .15])
    tgt_rotmat = rm.rotmat_from_axangle([0, 1, 0], math.pi * 2 / 3)
    # gm.gen_frame(pos=tgt_pos, rotmat=tgt_rotmat).attach_to(base)
    # gm.gen_frame(robot_s.arm.jnts[-1]['gl_posq'],robot_s.arm.jnts[-1]['gl_rotmatq'],length=1).attach_to(base)
    print(robot_s.arm.jnts[-1]['gl_posq'])
    print(robot_s.arm.jnts[-1]['gl_rotmatq'])
